...
public class RecurringNotif2 extends AppCompatActivity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_recurring_notif2);
        ...
        String timeDescrip = mPreferences.getString("time3a", "Set Time");
        String dateDescrip = mPreferences.getString("date3a", "Set Date");
...
// Apply the adapter to the spinner.
        ...
        int i = mPreferences.getInt("position3",-1);
        Log.d("MyActivity", "what" + i);
        if(mPreferences.getInt("position3",-1)>=0){
            spinner.setSelection(mPreferences.getInt("position3",0));
        ...
        mDateSetListener2 = new DatePickerDialog.OnDateSetListener() {
            ...
            public void onDateSet(DatePicker datePicker, int year2, int month2, int day2) {
                ...
                Date2.setText(date2);
                preferencesEditor.putString("date3a", date2);
        ...
        time.setOnClickListener(new View.OnClickListener() {
            ...
            @Override
            public void onClick(View v) {
                TimePickerDialog dialog3 = new TimePickerDialog(RecurringNotif2.this, mDateSetListener3,
                        hour, minute, false);
        ...
        mDateSetListener3 = new TimePickerDialog.OnTimeSetListener() {
            ...
            public void onTimeSet(TimePicker view, int hourOfDay, int minute) {
                ...
                time.setText(Time);
                preferencesEditor.putString("time3a", Time);
                preferencesEditor.apply();
    ...
    public void onItemSelected(AdapterView<?> adapterView, View view, int position, long id) {
        ...
        SharedPreferences.Editor preferencesEditor = mPreferences.edit();
        preferencesEditor.putInt("position3", position);
        preferencesEditor.apply();
    ...
    public void recurring(String def){
        if(def.equals("None")){
            //repeat(1);
            try {
                WorkManager.getInstance().cancelUniqueWork("MyActivity3a");
    ...
    public void repeat(int difference){
        periodicWork2a = new PeriodicWorkRequest.Builder(NotifyWorker3a.class, difference, TimeUnit.MINUTES)                .build();
        //WorkManager.getInstance().enqueue(periodicWork3);
        WorkManager.getInstance().enqueueUniquePeriodicWork("MyActivity3a", ExistingPeriodicWorkPolicy.KEEP, periodicWork2a);
        //WorkManager.getInstance().enqueue(periodicWork);
    }
    OneTimeWorkRequest mywork3a;

    public void runAt(Calendar cal) {

        Calendar calendar = Calendar.getInstance();
        long difference = cal.getTimeInMillis() - calendar.getTimeInMillis();
        Log.d("MyActivity", "running" + difference);
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {


            mywork3a =
                    new OneTimeWorkRequest.Builder(NotifyWorker3a.class)
                            ...
                            .build();
            WorkManager.getInstance().enqueueUniqueWork("mywork3a", ExistingWorkPolicy.KEEP, mywork3a);
            Log.d("MyActivity", "ran" + difference);
    ...
    public void CancelRemind5(View view) {
        try {
            WorkManager.getInstance().cancelUniqueWork("mywork3a");
            time.setText("Set Time");
            Date2.setText("Set Date");
            final SharedPreferences.Editor preferencesEditor = mPreferences.edit();
            preferencesEditor.putString("date3a", Date2.getText().toString());
            preferencesEditor.putString("time3a", time.getText().toString());
            preferencesEditor.apply();
        }catch (Exception NullPointerException){
            return;
